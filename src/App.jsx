 ...
 * Tabs: Inicio, Liga, Intereses, Mi equipo
 *
 * Data files in repo:
 *  - data/users.json
...
const AVAIL_LABEL = { AVAILABLE: "Disponible", LISTENING: "En escucha", NOT_AVAILABLE: "No disponible" };
const AVAIL_TONE = { AVAILABLE: "good", LISTENING: "neutral", NOT_AVAILABLE: "bad" };
...
function uid(prefix="u"){ return `${prefix}_${Math.random().toString(36).slice(2,10)}${Date.now().toString(36)}`; }
...
async function ghError(res) {
  ...
  let msg = text;
  try { const j = JSON.parse(text); msg = j?.message ? String(j.message) : text; } catch {}
...
async function ghPutFile(path, content, sha = null, message = null) {
  ...
  if (res.status === 409) { const e = await ghError(res); e.code = 409; throw e; }
  if (!res.ok) throw await ghError(res);
  return res.json();
}
...
async function ghPutJsonWithRetry(path, mutator, label) {
  return ghEnqueueWrite(async () => {
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      try { await ghPutJson(path, next, sha, label); return next; }
      catch (e) { if (e?.code === 409 && attempt === 0) continue; throw e; }
    }
  });
}
...
function normPos(p) {
  ...
  return ["QB","RB","WR","TE"].includes(up) ? up : up;
}
function normalizeRosterIds(roster) {
  ...
  return arr.map((x) => {
    ...
    if (typeof x === "object") return x.id != null ? String(x.id) : x.player_id != null ? String(x.player_id) : null;
    return null;
  }).filter(Boolean);
}
...
function GlobalStyles() {
  return (
    <style>{`
      ...
      body { margin: 0; background: ${COLORS.page}; color: ${COLORS.navy}; font-family: system-ui, -apple-system, Segoe UI, Roboto, Arial; }
      * { box-sizing: border-box; }
      ...
      .chip { padding: 7px 10px; border-radius: 999px; border: 1px solid ${COLORS.border}; background: ${COLORS.sky}; color: ${COLORS.navy}; font-weight: 800; font-size: 12px; cursor: pointer; }
  ...
  return (
    <div className="topbar">
      <div className="topbarInner">
        ...
        {me ? (
          <>
            <div className="chip breakAnywhere" style={{ cursor: "default" }}>{me.email}</div>
    ...
    <div className="playerRow">
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end" }}>
        {INTEREST_LEVELS.map((l) => (
          <Button key={l} variant={level === l ? "primary" : "ghost"} onClick={() => onSet(level === l ? "NONE" : l)} style={{ padding: "10px 12px" }}>
            {INTEREST_LABEL[l]}
          </Button>
        ))}
      </div>
...
function HomeView({ myIncoming, myOutgoing, byUser, playersById, picksCatalog }) {
  const pickLabel = useMemo(() => new Map(picksCatalog.map((p) => [String(p.id), p.label])), [picksCatalog]);
  const fmtAsset = (row) => {
    ...
    <div style={{ marginTop: 12 }}>
      <Card style={{ marginBottom: 12 }}>
        <div className="row" style={{ alignItems: "baseline" }}>
          ...
          <div className="muted" style={{ fontWeight: 900 }}>Tocá el estado: Disponible → En escucha → No disponible</div>
          <div className="spacer" />
          <div className="tabs">
          ...
          {mode === "players" ? (
            <>
              <div style={{ display: "grid", gap: 10 }}>
                <Input value={q} onChange={setQ} placeholder="Buscar jugador..." />
                <div className="seg">
                    ...
                    <div key={pid} className="playerRow">
                      <div style={{ display: "flex", gap: 10, alignItems: "center", minWidth: 0 }}>
                        ...
                        <div style={{ minWidth: 0 }}>
                          ...
                          <div className="muted" style={{ fontSize: 12, fontWeight: 900 }}>{normPos(p.position)} · {p.team || "-"} · ADP {p.adp_formatted || "-"}</div>
                        </div>
                      </div>
            ...
            <>
              ...
              <select
                ...
                style={{ padding:"14px 14px", borderRadius:14, border:`1px solid ${COLORS.border}`, background: COLORS.sky, color: COLORS.navy, fontWeight: 900 }}
              >
        ...
        <Card>
          {mode === "players" ? (
            <>
              ...
              <div className="muted" style={{ fontSize: 13, fontWeight: 900 }}>Asignación automática (QB/RB/WR/TE → FLEX → BN)</div>
                          ...
                          return (
                            <div key={pid} className="playerRow">
                              <div style={{ display: "flex", gap: 10, alignItems: "center", minWidth: 0 }}>
                                ...
                                <div style={{ minWidth: 0 }}>
                                  <div className="breakAnywhere" style={{ fontWeight: 1000 }}>{p.name}</div>
                                  <div className="muted" style={{ fontSize: 12, fontWeight: 900 }}>{normPos(p.position)} · {p.team || "-"}</div>
                                </div>
                              </div>
...
function LeagueView({ me, teams, byUser, playersById, picksCatalog, interests, onSetInterest }) {
  ...
  const selectedRoster = useMemo(() => {
    ...
    return ids.map((id) => playersById.get(String(id))).filter(Boolean);
  ...
  if (!selected) return <div style={{ marginTop: 12 }} className="muted">No hay otro equipo seleccionado.</div>;
      ...
      <div style={{ marginTop: 12 }} className="grid2">
        <Card>
          <div style={{ display:"grid", gap:6 }}>
            <div style={{ fontSize: 18, fontWeight: 1000 }}>{selected.display_name} {selected.team_name ? `— ${selected.team_name}` : ""}</div>
            <Pill>{selected.team_status || "—"}</Pill>
          </div>

          <div style={{ marginTop: 14 }}>
            <h3 style={{ marginTop: 0 }}>Jugadores</h3>
            <div className="muted" style={{ fontSize: 13, fontWeight: 900 }}>Marcá tu interés: Bajo / Medio / Alto</div>

            <div style={{ marginTop: 12 }} className="list">
              {selectedRoster.length === 0 ? <div className="muted">Sin roster cargado.</div> : null}
              {selectedRoster.map((p) => {
                const pid = String(p.player_id);
                const current = interests.find((x) => x.from_user_id===me.id && x.to_user_id===selected.user_id && x.asset_type==="PLAYER" && String(x.asset_id)===pid);
                return (
                  <InterestAssetRow
                    key={pid}
                    title={p.name}
        ...
        <Card>
          ...
          <div style={{ marginTop: 12 }} className="list">
            ...
            {selectedPicks.slice().sort().map((pid) => {
              const current = interests.find((x)=>x.from_user_id===me.id && x.to_user_id===selected.user_id && x.asset_type==="PICK" && String(x.asset_id)===String(pid));
              return (
                <InterestAssetRow
                  key={pid}
...
function InterestsView({ me, byUser, myOutgoing, myIncoming, playersById, picksCatalog }) {
  ...
  return (
    <div style={{ marginTop: 12 }} className="grid2">
      ...
      <Card>
        <h2 style={{ marginTop: 0 }}>Interesados en lo mío</h2>
        {myIncoming.length === 0 ? (
...
export default function App() {
  ...
  const [tab, setTab] = useState(() => localStorage.getItem("ftb_tab") || "team");
  ...
  useEffect(() => {
    ...
    if (!GH_TOKEN) setBootError("Falta VITE_GH_TOKEN.");
  ...
  const playersById = useMemo(() => new Map(players.map((p) => [String(p.player_id), p])), [players]);
  ...
  useEffect(() => {
    if (!me) return;
    (async () => {
      try { await Promise.all([refreshData(), loadPlayers()]); }
      catch (e) { setBootError(String(e?.message || e)); }
    })();
  }, [me?.id]);

  const byUser = useMemo(() => new Map(teams.map((t) => [t.user_id, t])), [teams]);
  const myRow = useMemo(() => (me ? byUser.get(me.id) : null), [byUser, me]);
  ...
  function friendlyAuthError(e) {
    ...
    if (status === 401) return "GitHub 401: token inválido.";
    if (status === 403) return "GitHub 403: sin permisos / rate limit.";
    if (status === 404) return "GitHub 404: faltan /data/*.json.";
  ...
  async function signup() {
    ...
    try {
      ...
      await ghPutJsonWithRetry(
        PATH_USERS,
        (cur) => [...cur, { id: userId, email: email.trim().toLowerCase(), pass_hash: pwHash, created_at: nowIso() }],
        "create user"
      );
  ...
  const rosterPlayers = useMemo(() => {
    return myRosterIds.map((id) => playersById.get(String(id))).filter(Boolean).map((p) => ({
      ...p, player_id: String(p.player_id), position: normPos(p.position),
    }));
        ...
        {!me ? (
          <Card>
            ...
            <div style={{ marginTop: 14, display: "grid", gap: 10 }}>
              ...
              <div className="muted" style={{ fontSize: 13 }}>Tip: 401/403 = token mal o sin permisos.</div>
            </div>
          </Card>
        ) : (
          <>
                ...
                <div style={{ display: "grid", gap: 10 }}>
                  <div className="row">
                    ...
                    <select
                      ...
                      style={{ padding:"14px 14px", minWidth:220, borderRadius:14, border:`1px solid ${COLORS.border}`, background: COLORS.surface, color: COLORS.navy, fontWeight: 900 }}
                    >
            ...
            {tab === "home" ? (
              ...
              <MyTeamView
                ...
                onTogglePlayerAvail={(pid) => updateMyTeam((t) => {
                  const avail = { ...(t.availability || {}) };
                ...
                onTogglePickAvail={(pickId) => updateMyTeam((t) => {
                  const avail = { ...(t.availability || {}) };
